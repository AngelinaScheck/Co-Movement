function FRETSTORM(fileName, thresh, windowSize, firstGreenFrame, TFORMpath, alternationPeriod, tif, alttype, startframe)
...
% image limits independent from setup (read-in wiht movie)
...
savename    = [fileName(1:end-5),'_thresh',num2str(thresh),'_lobject',num2str(windowSize),'_startframe', num2str(startframe),'_ALEX_FRET.out'];
%definitions for the output file
saveFileHeader = 'FRAME\tXCENTER\tYCENTER\tBRIGHTNESS\tBG\tI0\tS_X\tS_Y\tTHETA\tECC\tE\tS\n';
else
fprintf('\n\nCW FRETSTORM analysis for file %s\n',fileName);
savename    = [fileName(1:end-5),'_thresh',num2str(thresh),'_lobject',num2str(windowSize),'_startframe', num2str(startframe),'_CW_FRET.out'];    
%definitions for the output file
saveFileHeader = 'FRAME\tXCENTER\tYCENTER\tBRIGHTNESS\tBG\tI0\tS_X\tS_Y\tTHETA\tECC\tE\n';
end
...
bpasskerneldiametre = max(round(windowSize),1);%lobject from gaussStormGUI.m
...
if (alttype == 0)
    ...
    for nn = startframe:2:nFrame-1 % for each frame
      ...
      AAImFilt = bpass(AAIm,1,bpasskerneldiametre);

      % first position guesses in AA
      AAPos=findLocalMax(AAImFilt,thresh);

      % find corresponding DD position guesses with tform 
      DDPos = tforminv(tform,AAPos);
      ...
      for j = 1:size(AAPos,1)
        ...
        %Sigma can be filtered post-analysis
        sigmaLim = [0 inf];
        ...
        DDDAposThresh = windowSize/5;%windowSize/100;    
        [DDphot_count, DDa, DDnormChi2, DDposFinal, DDeccentricity] = freeGaussFitEllipse( DDIm, DDPos(j,:), windowSize,DDDAposThresh, sigmaLim, initguess,'autoDetect');
        [DAphot_count, DAa, DAnormChi2, DAposFinal, DAeccentricity] = freeGaussFitEllipse( DAIm, AAPos(j,:), windowSize,DDDAposThresh, sigmaLim, initguess,'autoDetect');

        E = DAphot_count / (DDphot_count + DAphot_count); %raw
        S = (DDphot_count + DAphot_count) / (DDphot_count + DAphot_count + AAphot_count);

        if ~all(AAa == 0 & DDa ==0 & DAa ==0)%if the fit has worked
          %for DD
          %need to write following:'FRAME\tXCENTER\tYCENTER\tPHOTONS\tBG\tI0\tS_X\tS_Y\tTHETA\tECC\tE\tS\n';
    ...
    for nn = startframe:1:nFrame % for each frame
      ...
      DDDAIm_sumFilt = bpass(DDDAIm_sum,1,bpasskerneldiametre);

      % find position guesses from summed image
      threshCW = thresh * 2; %because we have summed two images.
      DDDAPos=findLocalMax(DDDAIm_sumFilt,threshCW);

      % find corresponding DD position guesses
      DDPos = tforminv(tform,DDDAPos);

      % loop over positions
      for j = 1:size(DDDAPos,1)
        %do the fit
        initguess = [];
        DAposThresh = windowSize/2; % if the pos moves more than windowSize/2 from initialPos, drop it
        %Sigma can be filtered post-analysis
        sigmaLim = [0 inf];
        [DAphot_count, DAa, DAnormChi2, DAposFinal, DAeccentricity] = freeGaussFitEllipse( DAIm, DDDAPos(j,:), windowSize,DAposThresh, sigmaLim, initguess,'autoDetect');

        %do the corresponding DD and DA fits
        DDposThresh = windowSize/5;%windowSize/100;    
        [DDphot_count, DDa, DDnormChi2, DDposFinal, DDeccentricity] = freeGaussFitEllipse( DDIm, DDPos(j,:), windowSize,DDposThresh, sigmaLim, initguess,'autoDetect');
        ...
        if ~all(DDa ==0 & DAa ==0)%if the fit has worked
          %for DD
          %need to write following:'FRAME\tXCENTER\tYCENTER\tPHOTONS\tBG\tI0\tS_X\tS_Y\tTHETA\tECC\tE\n';
          fprintf(fid,'%d\t',nn); %FRAME
          fprintf(fid,'%6.3f\t',DDposFinal(1)); %XCENTER
          fprintf(fid,'%6.3f\t',DDposFinal(2)); %YCENTER
          fprintf(fid,'%6.3f\t',DDphot_count); %PHOTONS
