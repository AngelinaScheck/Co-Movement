function FRETSTORM(fileName, thresh, windowSize, firstGreenFrame, TFORMpath, alternationPeriod, tif)
...
% included into gaussStorm.m now 15/02/12 Anne Plochowietz

fprintf('\n\nFRET STORM analysis for file %s\n',fileName);

savename    = [fileName(1:end-5),'_thresh',num2str(thresh),'_lobject',num2str(windowSize),'.fretstorm.pos.out'];
...
%5ms alternation 192-260-->69; 10ms alternation 192-319-->128 (first green frame lim then red frame lim)
%imageLim = [257 512 1 69; 1 256 1 69]; %halfdome
%imageLim = [1 171 1 256; 172 342 1 256]; %zugspitze
...
%loop over frames
for nn = firstGreenFrame:2:nFrame-1 % for each frame
  ...
  for j = 1:size(AAPos,1)
    ...
    if ~all(AAa == 0 )%if the fit has worked
      %for DD
